...
async function game(): Promise<void> {
  ...
  while (playAgain) {
    let { maxNumber, numPlayers }: { maxNumber: number; numPlayers: number } =
    ...
    let leaderboard: { player: number; attempts: number }[] = [];
    let currentPlayer: number = 1;

    while (true) {
      let secretNumber: number = generateRandomNumber(1, maxNumber);
      let attempts: number = 0;
      let hintGiven: boolean = false;

      console.log(chalk.blue(`Player ${currentPlayer}, it's your turn to guess!`));

      let { guess }: { guess: number } = await inquirer.prompt([
        {
          type: 'number',
          name: 'guess',
      ...
      while (guess !== secretNumber) {
        ...
        if (!hintGiven) {
          let { hint }: { hint: boolean } = await inquirer.prompt([
            {
              type: 'confirm',
              name: 'hint',
          ...
          if (hint) {
            const hintMessage: string =
              secretNumber % 2 === 0 ? 'The secret number is even.' : 'The secret number is odd.';
    ...
    let { showLeaderboard }: { showLeaderboard: boolean } = await inquirer.prompt([
      {
        type: 'confirm',
    ...
    let { playAgainAnswer }: { playAgainAnswer: boolean } = await inquirer.prompt([
      {
        type: 'confirm',
